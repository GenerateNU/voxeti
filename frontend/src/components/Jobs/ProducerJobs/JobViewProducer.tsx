...
export default function JobViewProducer({ jobId } : JobViewProducerProps) {
  ...
  const ActionBar = ({ title, description, buttonText, status, onClick } : ActionBarProps) => {
    ...
    return (
      <div className='absolute left-0 top-0 w-full h-full bg-primary bg-opacity-75 z-10 text-background text-2xl text-center rounded-xl'>
        <div className='flex flex-col w-full h-full justify-center items-center'>
          ...
          >
            {(loading && jobState === 'ACCEPTED') ? <CircularProgress/> : 'Start Printing'}
          </StyledButton>
        </div>
      </div>
    )
      ...
      <section className='flex flex-col mt-10'>
        ...
        <div className='flex flex-row gap-x-5 items-center'>
          ...
          <div>
            ...
            <h2 className='text-xl font-semibold text-[#A4A4A4]'>
              Designer
            </h2>
          </div>
        </div>
