import { Container, Box, CircularProgress, IconButton, Fab } from "@mui/material";
...
export default function QuantityStep({
    ...
    const [currentQuantitiy, setCurrentQuantitiy] = useState(states.quantities[currentFile]);
    ...
    const incrementQuantity = (increment : number) => {
        setQuantities((quantities) => {
            ...
            quantities[currentFile] = (quantity == 1) && (increment < 0) ? 1 : quantity + increment
    ...
    return (
        <Container>
            <Box>
                <div className="font-semibold text-2xl mb-3">Quantities</div>
                <div className="text-lg text-[#777777] mb-10">
                    Preview your files and select quantities for each print
                </div>
            </Box>
            <div className='flex items-center justify-center w-[100%]'>
                <IconButton className='h-14 w-14 !p-0 !mr-2' onClick={() => nextFile(-1)}>
                ...
                <IconButton className='h-14 w-14 !p-0 !ml-2' onClick={() => nextFile(1)}>
